...
public class Renderer{
	...
	public static FrameBuffer copyeyebuffer = new FrameBuffer(Pixmap.Format.RGBA8888, (int) (Gdx.graphics.getWidth()/2f * fboscale), (int) (Gdx.graphics.getHeight() * fboscale),true);
    ...
    private static FrameBuffer finalbuffer1 = new FrameBuffer(Pixmap.Format.RGBA8888, (int)(Gdx.graphics.getWidth()*fboscale), (int)(Gdx.graphics.getHeight()*fboscale),true);
    private static FrameBuffer finalbuffer2 = new FrameBuffer(Pixmap.Format.RGBA8888, (int)(Gdx.graphics.getWidth()*fboscale), (int)(Gdx.graphics.getHeight()*fboscale),true);
    private static float lenscenteroffset;
    private static ShapeRenderer shapeRenderer = new ShapeRenderer();
    private static ModelBatch defaultBatch = new ModelBatch();
//	private static ModelBatch toonBatch = new ModelBatch(new DefaultShaderProvider(Gdx.files.internal("shader/default_toon/default_toon.vertex.glsl"), Gdx.files.internal("shader/default_toon/default_toon.fragment.glsl")));
	...
	public static void init(Camera cam) {
		...
		edgeshader = new ShaderProgram(Gdx.files.internal("shader/edge/edge.vertex.glsl"), Gdx.files.internal("shader/edge/edge.fragment.glsl"));
		blurshader = new ShaderProgram(Gdx.files.internal("shader/blur/blur.vertex.glsl"), Gdx.files.internal("shader/blur/blur.fragment.glsl"));
		glowshader = new ShaderProgram(Gdx.files.internal("shader/glow/glow.vertex.glsl"), Gdx.files.internal("shader/glow/glow.fragment.glsl"));
		lensshader = new ShaderProgram(Gdx.files.internal("shader/oculusrift/lens.vertex.glsl"), Gdx.files.internal("shader/oculusrift/lens.fragment.glsl"));
...
//			lenscenteroffset = (1.f - (or.getHMDInfo().LensSeparationDistance / 2.f / (or.getHMDInfo().HScreenSize / 4.f))) / 4.f;
...
//			}
		...
		glowbuffer1 = new FrameBuffer(Pixmap.Format.RGBA8888, framebufferwidth / glowfrac, framebufferheight / glowfrac,true);
		glowbuffer2 = new FrameBuffer(Pixmap.Format.RGBA8888, framebufferwidth / glowfrac, framebufferheight / glowfrac,true);
	...
	public static void renderCopyEyeBuffer(){
		...
		clearScreen(0,0,0,0);
		spriteBatch.begin();
		spriteBatch.draw(copyeyebuffertexreg
				, 0f, 0f
				, 0f, 0f
...
//		spriteBatch.end();
	...
	public static void clearViewport(Rectangle viewport){
		...
		Gdx.gl.glScissor((int)viewport.getX(), (int)viewport.getY(), (int)viewport.getWidth(), (int)viewport.getHeight());
		Gdx.gl.glClearColor(0, 0, 0, 0);
		Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT | GL20.GL_DEPTH_BUFFER_BIT);
	...
	public static void render(EntitySystem es, Camera cam, Environment env, Rectangle viewport, FrameBuffer target){
		if(target != null){
			if(useglow){
				...
				helpbuffer1.end();
				spriteBatch.begin();
				spriteBatch.draw(helpbuffer1.getColorBufferTexture(), viewport.getX()/fboscale, viewport.getY()/fboscale
						, viewport.getWidth()/fboscale, viewport.getHeight()/fboscale
						, 0, 0
						, helpbuffer1.getWidth(), helpbuffer1.getHeight()
						, false, true);
				spriteBatch.end();
	...
	private static void glowify(FrameBuffer input, FrameBuffer output){
		...
		glowbuffer1.begin();
		{
			...
			blurshader.begin();
			{
				...
				blurshader.setUniformf("size", (float)input.getColorBufferTexture().getWidth(), (float)input.getColorBufferTexture().getHeight());
		...
		glowbuffer2.begin();
		{
			...
			blurshader.begin();
			{
				...
				blurshader.setUniformf("size", (float)glowbuffer1.getColorBufferTexture().getWidth(), (float)glowbuffer1.getColorBufferTexture().getHeight());
				fullScreenTriangle.render(blurshader, GL20.GL_TRIANGLES, 0, 3);
			}
			blurshader.end();
		}
	...
	private static void renderOutlines(EntitySystem es, Camera cam, Environment env, Rectangle viewport, FrameBuffer target){
		...
		dest.begin();
		{
			...
			edgeshader.begin();
			{
				edgeshader.setUniformf("size", (float)src.getColorBufferTexture().getWidth(), (float)src.getColorBufferTexture().getHeight());
		...
		if(target != null){
			...
			spriteBatch.draw(dest.getColorBufferTexture(), viewport.getX(), viewport.getY()
					, viewport.getWidth(), viewport.getHeight()
					, 0, 0
					, (int)(dest.getWidth()), (int)(dest.getHeight())
					, false, true);
...
//		Gdx.gl20.glEnable(GL20.GL_DEPTH_TEST);
	...
	public static void renderMap(Camera cam, OverworldMap map, int currentNode, int selected){
		...
		Vector2 offset = new Vector2((cam.viewportWidth-map.mapsize.x)/2.f, (cam.viewportHeight-map.mapsize.y)/2.f);
//		Vector2 offset = new Vector2(0,0);
		...
		for(int i = 0; i < map.nodes.size(); ++i){
			for(int j = 0; j < map.nodes.size(); ++j){
				...
				if(map.adjmat.get(i).get(j)){
					shapeRenderer.line(map.nodes.get(i).position.cpy().add(offset), map.nodes.get(j).position.cpy().add(offset));
				}
			}
		}
		shapeRenderer.end();
	...
	public static void renderMouse(Camera cam){
		shapeRenderer.setProjectionMatrix(cam.combined);
		shapeRenderer.begin(ShapeType.Filled);
		Vector3 pos = new Vector3(Gdx.input.getX()-(Gdx.graphics.getWidth()-cam.viewportWidth)*0.5f, Gdx.graphics.getHeight()-Gdx.input.getY()-(Gdx.graphics.getHeight()-cam.viewportHeight)*0.5f, 0);
		shapeRenderer.setColor(Color.WHITE);
    ...
    private static Mesh createFullScreenTriangle() {
        ...
        Mesh tmpMesh = new Mesh(true, 3, 0, new VertexAttribute(Usage.Position, 2, "a_position"), new VertexAttribute(Usage.TextureCoordinates, 2, "a_texCoord0"));
